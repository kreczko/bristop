...
#include "SimDataFormats/GeneratorProducts/interface/GenEventInfoProduct.h" //genEventInfo
...
TreeBranch::value TreeBranch::branch(const edm::Event& iEvent){
  ...
  // TL added 3 lines
  else ANOTHER_VECTOR_CLASS(reco::Conversion);//TL
  else ANOTHER_VECTOR_CLASS(reco::VertexCompositeCandidate);//TL
  else ANOTHER_CLASS(GenEventInfoProduct);//TL
  else {
