...
const int n = 4;
const int nrange = 9;
const bool plot_average_est = false;

// free in 12j, fix in 34j
bool constrain_gaus_mean_12j = false;
bool show_free_fit_res_34j = false;
...
void plot_qcd_estimate(string func="gaus"){
  ...
  if(constrain_gaus_mean_12j) show_free_fit_res_34j = false;
  ...
  */
  ...
  for(int i=0; i < 4*nrange; ++i){
    ...
    if( i%4 <= 1 )  {//1,2j      
      if( fabs(est[i]) > max_dev_12j ) max_dev_12j = fabs(est[i]);
    } else if (i%4==2 ) { //3j
      if( fabs(est[i]) > max_dev_3j ) max_dev_3j = fabs(est[i]);
    } else if (i%4==3 ) { //4mj
      if( fabs(est[i]) > max_dev_4j ) max_dev_4j = fabs(est[i]);	
    }   
    if(i%4==0) cout << endl; //first of 4
    cout << i%4+1 <<"j   est[" << i << "] = " << est[i] << "   ";
    if(i%4<=1) cout << "max_dev_12j = " << max_dev_12j; //12j
    cout << endl;
  }
  result.close();
  ...
  for(int i=0; i < 4*nrange; ++i){
    ...
    // read in 3,4j only
    //cout << "amtb: i=" << i << endl;
    if( !constrain_gaus_mean_12j && (i%4 < 2) ) continue;
    
    result >> estFix[i];
    estFix2.push_back(estFix[i]);
    
    
    cout << i%4+1 <<"j   estFix[" << i << "] = " 
	 << estFix[i] << "   " <<endl;
    
    //cout << "amtb: i=" << i << endl;
    
    
    if( fabs(estFix[i]) > maxDevFix ) maxDevFix = fabs(estFix[i]);
    if( fabs(estFix[i]) > max_dev_34j_fix ) max_dev_34j_fix = fabs(estFix[i]);
    if( i%4==2 ) {//3j
      if( fabs(estFix[i]) > max_dev_3j_fix ) max_dev_3j_fix = fabs(estFix[i]);
    }else if( i%4==3 ){ //>=4j
      if( fabs(estFix[i]) > max_dev_4j_fix ) max_dev_4j_fix = fabs(estFix[i]);
    }
  }
  ...
  */
  ...
  if(func=="gaus")        h = new TH1D("h","Variation of QCD estimates with fit range (Gaussian)",4,0.5,4.5);
  else if(func=="pol3")   h = new TH1D("h","Variation of QCD estimates with fit range (Pol3)",4,0.5,4.5);
  else if(func=="landau") h = new TH1D("h","Variation of QCD estimates with fit range (Landau)",4,0.5,4.5);
  ...
  if(constrain_gaus_mean_12j) show_range = int(max_dev)+1;
  ...
  */
  ...
  if( func!="gaus" || !constrain_gaus_mean_12j ) {
    leg.AddEntry(gr1,"Free: 0.2-1.0","p");
    leg.AddEntry(gr2,"Free: 0.2-1.2","p");
    leg.AddEntry(gr3,"Free: 0.2-1.4","p");
    leg.AddEntry(gr4,"Free: 0.3-1.1","p");
    leg.AddEntry(gr5,"Free: 0.3-1.3","p");
    leg.AddEntry(gr6,"Free: 0.3-1.5","p");
    leg.AddEntry(gr7,"Free: 0.4-1.2","p");
    leg.AddEntry(gr8,"Free: 0.4-1.4","p");
    leg.AddEntry(gr9,"Free: 0.4-1.6","p");
  }
  if(func=="gaus" && constrain_gaus_mean_12j ) {
    leg.AddEntry(gr1,"#mu-constr.: 0.2-1.0","p");
    leg.AddEntry(gr2,"#mu-constr.: 0.2-1.2","p");
    leg.AddEntry(gr3,"#mu-constr.: 0.2-1.4","p");
    leg.AddEntry(gr4,"#mu-constr.: 0.3-1.1","p");
    leg.AddEntry(gr5,"#mu-constr.: 0.3-1.3","p");
    leg.AddEntry(gr6,"#mu-constr.: 0.3-1.5","p");
    leg.AddEntry(gr7,"#mu-constr.: 0.4-1.2","p");
    leg.AddEntry(gr8,"#mu-constr.: 0.4-1.4","p");
    leg.AddEntry(gr9,"#mu-constr.: 0.4-1.6","p");
  }
  leg.AddEntry(gr_1,"Constrained: 0.2-1.0","p");
  leg.AddEntry(gr_2,"Constrained: 0.2-1.2","p");
  leg.AddEntry(gr_3,"Constrained: 0.2-1.4","p");
  leg.AddEntry(gr_4,"Constrained: 0.3-1.1","p");
  leg.AddEntry(gr_5,"Constrained: 0.3-1.3","p");
  leg.AddEntry(gr_6,"Constrained: 0.3-1.5","p");
  leg.AddEntry(gr_7,"Constrained: 0.4-1.2","p");
  leg.AddEntry(gr_8,"Constrained: 0.4-1.4","p");
  leg.AddEntry(gr_9,"Constrained: 0.4-1.6","p");

  leg.Draw();
  if(func=="gaus")        c1.SaveAs("qcd_estimate_gaus.gif");
  else if(func=="pol3")   c1.SaveAs("qcd_estimate_pol3.gif");
  else if(func=="landau") c1.SaveAs("qcd_estimate_landau.gif");


  h->GetYaxis()->SetRangeUser( -1, 1 );
  c1.SaveAs(Form("qcd_estimate_%s_zoom_11.gif",func.c_str()));

  h->GetYaxis()->SetRangeUser( -2, 2 );
  c1.SaveAs(Form("qcd_estimate_%s_zoom_22.gif",func.c_str()));

  h->GetYaxis()->SetRangeUser( -3, 3 );
  c1.SaveAs(Form("qcd_estimate_%s_zoom_33.gif",func.c_str()));
  
  h->GetYaxis()->SetRangeUser( -6, 6 );
  c1.SaveAs(Form("qcd_estimate_%s_zoom_66.gif",func.c_str()));
  
  h->GetYaxis()->SetRangeUser( -8, 8 );
  c1.SaveAs(Form("qcd_estimate_%s_zoom_88.gif",func.c_str()));
  ...
  if(plot_average_est) {
    ...
    for(int i=0; i<nrange; ++i){
      ...
      average_3j += y[i][2];
      average_4mj += y[i][3];
      average_3j_fix  += yy[i][0];
      average_4mj_fix += yy[i][1];
    }
    ...
    TCanvas c2("c2","Average QCD estimate",610,10,600,600);
    c2.cd();
    c2.SetGrid(1,1);
    ...
    c2.SaveAs(Form("qcd_estimate_%s_average.gif",func.c_str()));
  }
