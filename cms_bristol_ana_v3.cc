...
const string mcname[17] = { "data", "ttbar", "QCD", "enri1", "enri2", "enri3", "bce1", "bce2", "bce3", "wj", "zj", "vqq",
		"singleTop", "tW", "tchan", "schan", "Zprime_M500GeV_W5GeV" };
const string mclabel[17] = { "data", "signal", "QCD", "enri1", "enri2", "enri3", "bce1", "bce2", "bce3", "W+jets", "Z+jets",
	...
	if (IsData()) {
		...
		for (size_t i = 0; i < mc_names.size(); ++i) {
			...
			if (nfiles.at(i) == 0)
				...
				cout << setprecision(4) << GetWeight(mc_names.at(i));
		...
		if (mc_seen.size() == 1)
			secondname = "mc_" + mc_seen.begin()->first; //one MC only
		else if (mc_seen.size() > 1)
...
void ana::SetMCFlag() {
	...
	if (nInitialEventMC["Zprime"] > 0)
		mc_sample_has_zprime = true;//FIXME: added
}
...
void ana::DefineCrossSection() {
	...
	if (m_LHCEnergyInTeV == 10) { //10TeV = default
		...
		cross_section["Zprime"] = 165; //FIXME: change to Z x-section
...
ana::ana() {
	...
	mc_sample_has_zprime = false;//FIXME: added
	isTTbar = false;
	isWjets = false;
	isZjets = false;
	isQCD = false;
...
//	const short nclass = 17; //data+mc


	// Add 17-9-09
	TDirectory *dir_DRemu = histf->mkdir("DRemu", "Delta R(e,mu)");
	dir_DRemu->cd();
	TH1F *h_DRemu_selE_GoodMu[mcsize];
	TH1F *h_DRemu_selE_GoodMu_pass[mcsize];
	...
	/*
	 ...
	 TH1F *h_nele[7][mcsize];
	 TH1F *h_ele_ET[4][7][mcsize];
	 TH1F *h_ele_eta[4][7][mcsize];
	 TH1F *h_ele_phi[4][7][mcsize];
	 TH1F *h_ele_iso[4][7][mcsize];
	...
	//TH1F *h_QCDest_NormCombRelIso[7][mcsize]; //"old" formulation (0-1)
	...
	const int nmctype(23+1); //extend to include wj, zj, QCD, VQQ, single top
	const int nstage(24+1); //add >=1Tele
	...
	for (Long64_t ev = 0; ev < nEvents; ++ev) {
		...
		if (!IsData() && isZjets && Nmc_doc > 0) {
			...
			if (isZee)
				h_Zee_Nphotons->Fill(Nphotons); //no cut
		...
		if (goodrun && fired_single_em && nGoodEle > 0 && this_met > METCUT && !isMuon && !isZ && !isConversion
			...
			reco_mttbar_using_M3(jets, electrons, met, nGoodIsoEle);
		}
...
void ana::PrintErrorTables(const double e_plus_jet[][5][24], const double e_plus_jet_weighted[][5][24], vector<string> ve) const {

	const int mynstage = 11; //up to !DIFFZ

	double e_plus_jet_errors[mynstage][5][25];
	...
	string kIndexmcNames[24] = { "", ttsample, ttsample, ttsample, ttsample, ttsample, ttsample, ttsample, ttsample, ttsample,
...
//----------------------------------------------------------------------------------
void ana::PrintError_NjetVcut(ofstream& myfile, const double e_plus_jet_weighted[][5][24],
		const double e_plus_jet_errors[][5][25], vector<string>& ve) const {
...
void ana::fillHistoNjet2D(TH1F* h[][17], const int ec, const float value, const double w) {
...
void ana::addHistoDataAndMC(TH1F* h[], const string name, const string title, const int nbin, const float xlow, const float xup) const {
	...
	if (!IsData())
		nhisto = 17; //MC//FIXME: changed from  16 to 17

	for (short i = 0; i < nhisto; ++i) {
		// only do for mc present in input
		if (i > 0 && is_mc_present(i) == false)
			continue;
		char hname[70];
		char htitle[100];
	...
	for (short i = 0; i < 3; ++i) {
		...
		if (!IsData()) { //MC
			if (isTTbar) {//ttbar
				...
				h = (TH1F*) histf->Get(Form("%s__Zprime", hname));
				if (h > 0)
					h->Fill(value, w);
			}
		}//if run on MC
	...
	for (short i = 0; i < 3; ++i) {
		...
		if (!IsData()) { //MC
			if (isTTbar) {//ttbar
				...
				h = (TH2F*) histf->Get(Form("%s__Zprime", hname));
				if (h > 0)
					h->Fill(v1, v2, w);
			}
		}//if run on MC
...
// NEW
//----- my method to add a set of 1D histograms (for each type of MC when running on MC) ------
void ana::addHisto_Njet_DataAndMC(TH1F* h[7][17], const string name, const string title, const int nbin, const float xlow,
		const float xup) {
...
//-------------- my method to fill  1D  histograms acc to njet & mctype ------------------
void ana::fillHisto_Njet_DataAndMC(TH1F* h[7][17], const float value, const double w) {
...
//----------------------------------------------------------------------------------------
void ana::DrawEventPerNjetTable(const double nevent[][5][24], const vector<string>& ve) const {
...
//----------------------------------------------------------------------------------------
void ana::DrawMCTypeTable(const double nevent[14][5][24], const string title, vector<string> ve) const {
	...
	for (short i = 0; i < 11; ++i) {//loop over cuts (up to HT)
		...
		//Zprime
		totalT = 0;
		for (short k = 23; k < 24; ++k) { //loop over zprime mc types (23)
			for (short j = njbegin; j < ntjet; ++j) {
				totalT += nevent[i][j][k];
			}
		}
		cout << " & " << setw(12) << fixed << totalT;
		totalA += totalT; //add zprime to total
...
//----------------------------------------------------------------------------------------
void ana::DrawSignalBGTable(const double nevent[][5][24], vector<string> ve) const {
...
void ana::DrawQCDTable(const double nevent[14][5][24], const string QCDtitle, vector<string> ve) const {
...
void ana::DrawSingleTopTable(const double nevent[14][5][24], const string title, vector<string> ve) const {
...
void ana::DrawSignalAcceptanceTable(const double nevent[][5][24], vector<string> ve) const {
...
bool ana::is_mc_present(const int code) const {
	...
	case 2:
		...
		break;
	case 3:
		return mc_sample_has_enri1;
		break;
	case 4:
		return mc_sample_has_enri2;
		break;
	case 5:
		return mc_sample_has_enri3;
		break;
	case 6:
		return mc_sample_has_bce1;
		break;
	case 7:
		return mc_sample_has_bce2;
		break;
	case 8:
		return mc_sample_has_bce3;
		break;
	case 9:
...
void ana::bookHistograms() {
	for (short int i = 0; i < mcsize; i++) {
		string histname;
		histname = mclabel[i] + "_" + "mttbar";
		//TODO: change 10000 to whatever the CoM energy is.
		hists_[histname.c_str()] = new TH1F(histname.c_str(), "reconstructed M_{ttbar}", 10000, 0, 10000);
...
void ana::reco_mttbar_using_M3(const vector<TLorentzVector>& jets, const vector<TLorentzVector>& eles, const std::vector<
		TLorentzVector>& met, const int nGoodIsoEle) {
	if (nGoodIsoEle > 0) {
		TLorentzVector blep, thad, tlep;
		TLorentzVector neutrino1, neutrino2;
		double et, px, py;
		et = met[0].Et();
		px = met[0].Px();
		py = met[0].Py();
		pair<double, double> pznu = compute_neutrino_momentum_z(et, eles[0]);
		double pz1, pz2;
		pz1 = pznu.first;
		pz2 = pznu.second;
		//		cout << "first root: " << pz1 << endl;
		//		cout << "second root: " << pz2 << endl;
		...
		TLorentzVector W1 = TLorentzVector(neutrino1 + eles[0]);
		TLorentzVector W2 = TLorentzVector(neutrino2 + eles[0]);
		//		cout << "W_1 mass_t:" << W1.Mt() << "\t" << "W_2 mass_t:" << W2.Mt() << endl;
		//		cout << "W_1 mass:" << W1.M() << "\t" << "W_2 mass:" << W2.M() << endl;
		if (W1.M() > 0 && W2.M() > 0) {
			if (fabs(W1.M() - 80) < fabs(W2.M() - 80)) {
				if (isZprime){
					hists_["Zprime_recoWmass"]->Fill(W1.M());
					hists_["Zprime_neutrino_pz"]->Fill(pz1);
				}
				else if (isTTbar){
					hists_["signal_neutrino_pz"]->Fill(pz1);
					hists_["signal_recoWmass"]->Fill(W1.M());
				}
			} else {
				if (isZprime){
					hists_["Zprime_neutrino_pz"]->Fill(pz2);
					hists_["Zprime_recoWmass"]->Fill(W1.M());
				}
				else if (isTTbar){
					hists_["signal_neutrino_pz"]->Fill(pz2);
					hists_["signal_recoWmass"]->Fill(W2.M());
				}
			}
		}
		for (unsigned int x = 0; x < Nmc_doc; x++) {
			...
			{//electron neutrino from W-decay from top-quark
				if (isZprime)
					hists_["Zprime_neutrino_pz_mc"]->Fill(mc_doc_pz->at(x));
				else if (isTTbar)
					hists_["signal_neutrino_pz_mc"]->Fill(mc_doc_pz->at(x));
			}

		}

		//		pair<double, double> newton= compute_neutrino_momentum_z2(met[0], eles[0]);
		//		double newpz = newton.first;
		//		neutrino1.SetPz(newpz);
		//		neutrino1.SetE(px * px + py * py + newpz * newpz);
		//		TLorentzVector W3 = TLorentzVector(neutrino1 + eles[0]);
		//		cout << "newton W mass_t: " << W3.Mt() << endl;
		//		cout << "newton W mass: " << W3.M() << endl;
	}
	//		TLorentzVector electron = eles[0];
	//		pair<vector<double>, vector<unsigned int> > res;
	//		res = compute_M3_modified(jets);
	//		double m3 = res.first[0];
	//		double maxpt = res.first[1];
	//		vector<unsigned int> which_3jet = res.second;
	//		double mttbar(-1);
	//
	//		vector<TLorentzVector> selected_jets;
	//		vector<TLorentzVector> rejected_jets;
	//		for(unsigned int i = 0; i< jets.size(); i++){
	//			if (i != which_3jet[0]
	//			 && i != which_3jet[1]
	//			 && i!= which_3jet[2]){
	//				rejected_jets.push_back(jets[i]);
	//			}
	//			else{
	//				selected_jets.push_back(jets[i]);}
	//		}
	//		unsigned int j = findClosest(rejected_jets, electron);
