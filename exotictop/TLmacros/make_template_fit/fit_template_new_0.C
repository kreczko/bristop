...
const int nj = 4;
const string jetbin_to_use_if_using_one_template = "4j";
...
void fit_template_new( string control_sample ="B3_e20",
  ...
  for(int j=0; j<4; j++) {
    ...
    TH1F *control = (TH1F*)f.Get(Form("QCD_planB/QCDest_CombRelIso_AES_plan%s_%s__data",
  ...
  */
    ...
    */
    ...
    res->AddText(Form("Est = %.1f",    nQCDInSignal_est[j]  ));
  ...
  printf("\n--------------------------------------------------\n");
  printf("         njet        True         Est         Dev\n");
  for(int jj=0; jj<4; jj++){
    printf(" %12s  %10.2f  %10.2f  %10.2f\n", 
	   njlabel[jj], nQCDInSignal_true[jj], nQCDInSignal_est[jj], Deviation[jj] );   
  }
  printf("----------------------------------------------------\n");
  ...
  string out = Form("MLfit__%s__4T__r%.1fto%.1f", 
		    control_sample.c_str(), fit_from, fit_upto);
  if(ntemp==1) {
    out = Form("MLfit__%s__1T%s__r%.1fto%.1f", 
  ...
  ofstream myfile;
  myfile.open(Form("est_template_%s_%dT.txt",
		   control_sample.c_str(),ntemp), ios::app); //out:(overwrite); ios:app (append)
