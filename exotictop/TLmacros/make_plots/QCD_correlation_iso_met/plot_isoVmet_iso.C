...
// 2) project "iso" from 2D: noMET, loMET, hiMET.
// permutation: nj: 1-4j
//              proc: enri1,2,3, bce1,2,3, QCD
//
// NES stage   2D   <------- ISO ------>
//       1     2D   noMET loMET hiMET
//       2      x     x     x     x
//       3
//---------------------------------------------
/*
...
//--------------------------------------------------------------------------------
// note: 
// plot_isoVmet_iso() -> plot_iso() -> plot_njet() -> do_plot_isoVmet()
//--------------------------------------------------------------------------------
void plot_isoVmet_iso(string etaRegion_user="both",string metType_user="t1"){
...
void do_plot_isoVmet(string sel_user = "enri1") {
  ...
  string opname = Form("isoVmet_iso_%s_%s", nj.c_str(), sel.c_str());
  if(etaRegion!="both")
    opname = Form("isoVmet_iso_%s_%s_%s", etaRegion, nj.c_str(), sel.c_str());
  ...
  if(debug) cout << "getting 1D histo" << endl;
  /*
  for(int i=0; i<nh; i++){
    //    cout << i << endl;
    h[i] = (TH1D*)f.Get( Form("%s_%s__%s",histo[i].c_str(), nj.c_str(), sel.c_str()) );
    // h[i]->ls();
    //h[i]->SetTitle( Form("%s", title[i].c_str()) );
    if(h[i]>0) {
      h[i]->GetXaxis()->SetTitle("RelIso");
      //if(h[i]->GetEntries()<=1000) rb=5;
      //if(h[i]->GetEntries()<=500) rb=10;
      h[i]->Rebin(rb);
...
//     if(plot_2D_unweighted)
//       h2[i] = (TH2F*)f.Get( Form("%s_%s__%s",histo2_uw[i].c_str(), etaRegion ,nj.c_str(), sel.c_str()) );
//     else
//       h2[i] = (TH2F*)f.Get( Form("%s_%s__%s",histo2[i].c_str(), etaRegion,   nj.c_str(), sel.c_str()) );
    ...
    // project met
    h2[i]->ProjectionX("iso");
    h[npro] = (TH1D*)iso->Clone();
    //    cout << "amtb 1" << endl;

    //    const int met_cut_bin = 30;
    const int met_nbin = h2[i]->GetNbinsY();

    //    h2[i]->ProjectionY("met",1,5);//low iso
    h2[i]->ProjectionX("iso",1,30);//low met 30 GeV
    h[npro+1] = (TH1D*)iso->Clone();

    //    h2[i]->ProjectionY("met",6,101);//high iso
    h2[i]->ProjectionX("iso",31, met_nbin+1);//high met
    h[npro+2] = (TH1D*)iso->Clone();
  ...
  for(int n=1; n<=npad; n++){
    ...
    if(h[i]>0) {
      h[i]->GetXaxis()->SetRangeUser(0,iso_max);
      h[i]->SetXTitle("RelIso");
      //if(n%4==2) h[i]->SetTitle("RelIso<0.1"); //2nd column
      if(n%4==3) h[i]->SetTitle("MET<30 GeV"); //3rd column
      if(n%4==0) h[i]->SetTitle("MET>30 GeV"); //4th column
      h[i]->Draw();
    }
    i++;    
  }
