...
//        6) Maybe add fillHisto_Njet_DataAndMC overloaded to without this_weight added
//        7) After HCP update, remove robustLoose, loose and tight IDs as they are defunct.
//        8) Do to alter DoBtagging so that the jets conform exactly to those selected (ie. add additional
//           id, and sort out ssvHE (for data >=36X, ssvHE, for mc in 35X, it's secVertex)
//#====================================================#
//# Last update:
//
//    ******  Need to add to AN stage, a njet plot for all njets, not just to 4 ****
//            Need to do: - Log numbers from inverted Zveto cut
//                        - Try and extend njet-sample by cut table with sum(tt), sum(qcd), sum(pj).
//                          Also, produce cut vs jet by sample, as well as sample versus jet by cut
//                        - Extend cutflow array to add a 5 or more jet bin (and 4 if not already in) 
//                        - Change error tables to move 4jets cut to after conversion
//                        - Also add photon+jets to error tables, and jet cuts 3,4,4mj,5,5mj
//
//  31 Oct 2010: Interesting_events.txt now only contains isolated cases for >1jets (to reduce content)
//
//  24 Oct 2010: Changed HLT run range due to new trigger menu from 148819 onwards. This will need to be updated 
//               when new samples with new trigger arrive with HLT_Ele22_SW_TighterEleId_L1R_v2
//
//  11 Oct 2010: Moved missing hits cut to after z veto
//               Put temp solution to DoBtagging section for non calo jets
//
//  10 Oct 2010: Remove trigger requirement for MC
//               Added back the dEtaIn cut on the electronIDs 
//
//  08 Oct 2010: Changed abs(PVz) requirement to 24 for MC and Data.
//               Added (pvz - Elz) < 1 requirement to the definition of an electron
//
//  04 Oct 2010: Change HLT paths to electron triggers. The HLT path for MC still needs to be addressed once it is decided upon.
//
//  29 Sept 2010: Fixed bug in PF leading jet AN plots.
//
//  17 Sept 2010: Added tr_conversionPartnerTrack_MHits variable to convTree to get number missing layers of just partner track alone. 
//  16 Sept 2010: I've extended the AN_et and AN_mtw plots to 1000 GeV to see clearly where overflow events are.
//
//  1 Sept 2010: -Made several changes to EstimateWjets section to run on data. Several objects that were only declared for MC 
//               were being accessed when using data.
//               - Add skim efficiencies  
//
// 31 Aug  2010: added some plots to AN to compare calo and pf jets pt spectrum. Needed to add calo jet branches when using
//               pfjets. 
//
// 16 Aug  2010: - mtw, ElzPVz, and AN_aes plots added. Added ElzPVz to interesting events
//               - Applied conversion veto to AN plots (they weren't being plotted after ALL selection cuts
// 15 Aug  2010: - Added myfile4 which outputs events for data after conversion veto
//               - Overloaded fillHisto_Njet_DataAndMC for two arguments without weight (default this_weight)
//               - Fixed bug in conversion finder
//
// 12 Aug  2010: - Changed size of conversionarray - need to add pj to conversionTable
//               - Replaced conversionFinder with conversionFinder2, and inserted new conversionFinder based on egamma one,
//                 with different way of looping over tracks, and larger deltaR (0.5).  
//
//               Do stuff for synchHCP: conv finder 0.02,0.02, PV (24), add veto, useless section, triggers
//               vbtf 95 for z veto, remove EE delEta cut
...
//               are local variables with the same name.
...
#include "cms_bristol_ana_v5.hh" // defines ana class, including branches and leaves for tree
#include "fb_cms.hh"
#include "fb_cms.cc"
...
void ana::PrintCuts() const {
  ...
  cout << "   For Data in Runs >= 140042, use HLT_Photon15_Cleaned_L1R."<< endl;
...
void ana::SetEventWeightMap(){ //only if run on MC
   ...
   if( m_runOnMyHLTskim ) {
     ...
     skimEffMap["ttjet"]  = 638765.0 / 1459404.0; 
     skimEffMap["wjet"]   = 2153929.0 / 10068895.0;
     skimEffMap["zjet"]   =  329061.0 /  1084921.0;

     skimEffMap["enri1"]  = 2126432.0 / 33801839.0;
     skimEffMap["enri2"]  = 5417656.0 / 40785278.0;
     skimEffMap["enri3"]  = 1482956.0 / 5546413.0;

     //skimEffMap["bce1"]   =  / 
     skimEffMap["bce2"]   = 442459.0 / 2475597.0;
     skimEffMap["bce3"]   = 578395.0 /  1208674.0;
     skimEffMap["pj1"]    = 311575.0 / 2255228.0;
     skimEffMap["pj2"]    = 240924.0 / 1068393.0;
     skimEffMap["pj3"]    = 361343.0 / 1025198.0;
     
     skimEffMap["tchan"]  =  192728.0 / 528593.0 ;
     skimEffMap["tW"]     =  181504.0 / 474437.0;

     /*
     cout << "\n Run on Spring10 e20 skim" << endl;
     
     skimEffMap["ttjet"]  =  642707. / 1468404. ; //
     skimEffMap["zjet"]   =   329061. / 1084921.; //
     skimEffMap["wjet"]   = 1101679. / 5143895. ; //

     skimEffMap["enri1"]  = 917274. / 14607326. ; //for HCP update     
     skimEffMap["enri2"]  = 3639776. / 27403259. ; //for HCP update   
     skimEffMap["enri3"]  = 1253958. / 4686413. ; //v4

     skimEffMap["bce1"]   =  86392. / 2606023. ; //v4
     skimEffMap["bce2"]   =  441537. / 2470597. ; //v4
     skimEffMap["bce3"]   =  477762. / 998674. ; //v4

     skimEffMap["pj1"]    =  311575. / 2255228. ; //v4
     skimEffMap["pj2"]    =  241590. / 1071393. ; //v4
     skimEffMap["pj3"]    =  338407. / 960198. ; //v4
     */
     /*comment out for now 
     ...
     skimEffMap["enri2"]  = 2991473. /  22528525. ; //for HCP update
...
void ana::ReadSelectedBranches() const {
   ...
   chain->SetBranchStatus("els_vz",1);
   ...
   if(m_jetAlgo=="Default"){ //using default jet-met
     ...
     if(IsData())chain->SetBranchStatus("jets_btag_ssvHE",1);

   } else if (m_jetAlgo=="pfjet") { //PFJet
     chain->SetBranchStatus(Form("NPFJets2"),1); //jets
     chain->SetBranchStatus("PFJets2_px",1);
     chain->SetBranchStatus("PFJets2_py",1);
     chain->SetBranchStatus("PFJets2_pz",1);
     chain->SetBranchStatus("PFJets2_energy",1);
     chain->SetBranchStatus("PFJets2_eta",1);
     chain->SetBranchStatus("PFJets2_pt",1);
     chain->SetBranchStatus("PFJets2_id_hitsInN90",1);
     chain->SetBranchStatus("PFJets2_id_fHPD",1);
     chain->SetBranchStatus("PFJets2_emf",1);
     chain->SetBranchStatus("PFJets2_numberOfDaughters",1);
     chain->SetBranchStatus("PFJets2_CEF",1);
     chain->SetBranchStatus("PFJets2_NHF",1);
     chain->SetBranchStatus("PFJets2_NEF",1);
     chain->SetBranchStatus("PFJets2_CHF",1);
     chain->SetBranchStatus("PFJets2_chg_Mult",1);
   ...
   chain->SetBranchStatus("tracks_d0dum",1);
   ...
   if(GetTrigger()) {
     ...
     //Since in data we requrie a variety of triggers, ignore trigger bit requirement, add manually
     //chain->SetBranchStatus(HLTBit.c_str(),1);
     if(IsData()){
       chain->SetBranchStatus("HLT_Ele10_LW_L1R",1);
       chain->SetBranchStatus("HLT_Ele15_LW_L1R",1);
       chain->SetBranchStatus("HLT_Ele15_SW_L1R",1);
       chain->SetBranchStatus("HLT_Ele15_SW_CaloEleId_L1R",1);
       chain->SetBranchStatus("HLT_Ele17_SW_CaloEleId_L1R",1);
       chain->SetBranchStatus("HLT_Ele17_SW_TightEleId_L1R",1);
       chain->SetBranchStatus("HLT_Ele22_SW_TighterEleId_L1R_v2",1);
     }
     ...
     if(m_runOnV4ntuples) {
       ...
       chain->SetBranchStatus("pass_photon10_TO20",1);     
       if(IsData()) {chain->SetBranchStatus("pass_photon15_TO20",1);     
	 chain->SetBranchStatus("pass_photon15clean_TO20",1);     
       }
...
void ana::CheckAvailableJetMET(){
   ...
   for ( int i = 0; i < list->GetEntries(); ++i) {
     ...
     else if(name=="NPFJets2")     availableJET.push_back("PF(AK5)");
...
void ana::BookHistograms_conv(){
   ...
   if(m_doPostCutPlots){
    addHisto_Njet_DataAndMC( AN_et,       "AN_et",     "AN_et",      1000,0,1000);
    addHisto_Njet_DataAndMC( AN_eta,       "AN_eta",     "AN_eta",      100,-2.5,2.5);
    addHisto_Njet_DataAndMC( AN_d0,       "AN_d0",     "AN_d0",      1000,0,1);
    addHisto_Njet_DataAndMC( AN_mh,       "AN_mh",     "AN_mh",      10,-0.5,9.5);
    addHisto_Njet_DataAndMC( AN_iso,       "AN_iso",     "AN_iso",      1000,0,10);
    addHisto_Njet_DataAndMC( AN_trkiso,       "AN_trkiso",     "AN_trkiso",      100,0,10);
    addHisto_Njet_DataAndMC( AN_ecaliso,       "AN_ecaliso",     "AN_ecaliso",      100,0,10);
    addHisto_Njet_DataAndMC( AN_hcaliso,       "AN_hcaliso",     "AN_hcaliso",      100,0,10);
   ...
   addHistoDataAndMC( AN_1leadPFJet_pt,       "AN_1leadPFJet_pt",     "AN_1leadPFJet_pt",      500,0,500);
   addHistoDataAndMC( AN_2leadPFJet_pt,       "AN_2leadPFJet_pt",     "AN_2leadPFJet_pt",      500,0,500);
   addHistoDataAndMC( AN_3leadPFJet_pt,       "AN_3leadPFJet_pt",     "AN_3leadPFJet_pt",      500,0,500);
   addHistoDataAndMC( AN_4leadPFJet_pt,       "AN_4leadPFJet_pt",     "AN_4leadPFJet_pt",      500,0,500);
   addHistoDataAndMC( AN_1leadJet_pt,       "AN_1leadJet_pt",     "AN_1leadJet_pt",      500,0,500);
   addHistoDataAndMC( AN_2leadJet_pt,       "AN_2leadJet_pt",     "AN_2leadJet_pt",      500,0,500);
   addHistoDataAndMC( AN_3leadJet_pt,       "AN_3leadJet_pt",     "AN_3leadJet_pt",      500,0,500);
   addHistoDataAndMC( AN_4leadJet_pt,       "AN_4leadJet_pt",     "AN_4leadJet_pt",      500,0,500);

   addHisto_Njet_DataAndMC( AN_met,       "AN_met",     "AN_met",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_HT,       "AN_HT",     "AN_HT",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_HTlep,       "AN_HTlep",     "AN_HTlep",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_M3,       "AN_M3",     "AN_M3",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_M3MaxPT,       "AN_M3MaxPT",     "AN_M3MaxPT",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_mtw,       "AN_mtw",     "AN_mtw",      1000,0,1000);
   addHisto_Njet_DataAndMC( AN_aes_reliso,       "AN_aes_reliso",     "AN_aes_reliso",      1000,0,10);
   addHisto_Njet_DataAndMC( AN_ElzPVz,       "AN_ElzPVz",     "AN_ElzPVz",      100000,0,100);
   addHisto_Njet_DataAndMC( AN_SigmaIetaIeta,       "AN_SigmaIetaIeta",     "AN_SigmaIetaIeta",      1000,0,0.1);
   addHisto_Njet_DataAndMC( AN_DelEta,       "AN_DelEta",     "AN_DelEta",      100,-0.05,0.05);
   addHisto_Njet_DataAndMC( AN_DelPhi,       "AN_DelPhi",     "AN_DelPhi",      1000,-0.5,0.5);
   addHisto_Njet_DataAndMC( AN_HoE,       "AN_HoE",     "AN_HoE",      200,0,0.2);
   addHisto_Njet_DataAndMC( AN_EE_SigmaIetaIeta,       "AN_EE_SigmaIetaIeta",     "AN_EE_SigmaIetaIeta",      1000,0,0.1);
   addHisto_Njet_DataAndMC( AN_EE_DelEta,       "AN_EE_DelEta",     "AN_EE_DelEta",      100,-0.05,0.05);
   addHisto_Njet_DataAndMC( AN_EE_DelPhi,       "AN_EE_DelPhi",     "AN_EE_DelPhi",      1000,-0.5,0.5);
   addHisto_Njet_DataAndMC( AN_EE_HoE,       "AN_EE_HoE",     "AN_EE_HoE",      200,0,0.2);

   }

   //ttbarMomentum = new TH2D("ttbarMomentum","ttbarMomentum",1000,0,1000,100,0,10);
...
void ana::BookHistograms_wj(){
   ...
   h_hadTop_maxPT_mass_3j = new TH1D("hadTop_maxPT_mass_3j", "had top mass (m3) (3j)",  nbm3,0,nbm3);
   h_hadTop_maxPT_pt_3j   = new TH1D("hadTop_maxPT_pt_3j",   "had top highest PT (3j)", 100,0,1000);
   // m3 for background region (non-isolated ele)                                                                            
   h_hadTop_maxPT_mass_nonIso_3j = new TH1D("hadTop_maxPT_mass_nonIso_3j", "had top mass (m3) (3j) nonIsoE", nbm3,0,nbm3);
   h_hadTop_maxPT_pt_nonIso_3j   = new TH1D("hadTop_maxPT_pt_nonIso_3j",   "had top highest PT (3j) nonIsoE", 100,0,1000);
   h_hadTop_maxPT_mass_3j->Sumw2();
   h_hadTop_maxPT_pt_3j  ->Sumw2();
   h_hadTop_maxPT_mass_nonIso_3j->Sumw2();
   h_hadTop_maxPT_pt_nonIso_3j  ->Sumw2();
   ...
   addHistoDataAndMC( h_m3_3j,              "m3_3j",              "m3_3j",       nbm3,0,nbm3); //960 global   
   addHistoDataAndMC( h_m3_control_3j,      "m3_control_3j",      "m3_3j (CR)",  nbm3,0,nbm3); //960 global   
...
//////// Plots above to be reviewed
     ...
     myfile5.open("IsoEvents.txt",ios::trunc); //over-write
     //myfile4.open("passEvents.txt",ios::trunc); //over-write     
     if(IsData()) {
     myfile2.open("interesting_events.txt",ios::trunc); //over-write     
     myfile3.open("reliso_list.txt",ios::trunc); //over-write
     myfile4.open("passEvents.txt",ios::trunc); //over-write
     //summaryOutput.open("summaryOutput.txt",ios::trunc); //over-write
   }
   ...
   ve2.insert( ve2.begin()+m_muonCutNum+5, Fourjets );
   ...
   //vector<TLorentzVector> iso_RL_electrons;//RL //taken out 18-08-10
   ...
   for(Long64_t ev=0; ev<nEvents; ++ev) {
     ...
     /*
     for(int i=0;i<Nmc_doc;i++){
       if( fabs(mc_doc_id->at(i)) == 6 ){
	 float myMomen = sqrt(mc_doc_pt->at(i)*mc_doc_pt->at(i) + mc_doc_pz->at(i)*mc_doc_pz->at(i) );
	 ttbarMomentum->Fill( myMomen,fabs(mc_doc_eta->at(i)) );
	   }
     }
     */
     ...
     //if(!NoScrapingFilter()){cout<<"run: "<<run<<", event: "<<event<<", Ntracks: "<<Ntracks<<endl;}
...
//        }
	 ...
	 if (jetsJPTAK5_pt->at(i) > JET_PTCUT &&
       ...
       for(unsigned int i = 0; i<NPFJets2; ++i) {
	 if (PFJets2_pt->at(i) > JET_PTCUT &&
	     fabs(PFJets2_eta->at(i)) < 2.4 && PFJets2_numberOfDaughters->at(i) > 1 &&
	     PFJets2_CHF->at(i) > 0 && PFJets2_chg_Mult->at(i) > 0 ) {// NB: EMF cannot be calculated for PFJets2
	   if(PFJets2_CEF->at(i) >= 0.99) continue;
	   if(PFJets2_NHF->at(i) >= 0.99) continue;
	   if(PFJets2_NEF->at(i) >= 0.99) continue;

	   TLorentzVector jt(PFJets2_px->at(i),PFJets2_py->at(i),PFJets2_pz->at(i),PFJets2_energy->at(i));
	   nGoodJet++;
	   jets.push_back(jt);
	 }
       }
...
//        this_met_phi = metsSC7_phi->at(0);
	 ...
	 //changed 120710, to allow for bit pattern in data. 1 passes ID, 3 passes ID+Iso. 3 not subset of 1
         ...
         if(!passEleID_VBTF_W95(j))continue;
     ...
     //------------------------------------------------------------------
     if( IsData() && iso_electrons.size() > 0 && ntj>1) {
       ...
       myfile2 << " electron     ET:  " << els_et->at(index_selected_ele) << " GeV" << endl;
       myfile2 << "             eta:  " << els_eta->at(index_selected_ele) << " , sc_eta: " << els_scEta->at(index_selected_ele) ;
       if( eleInBarrel(index_selected_ele) ) myfile2 << " (in BARREL)"<< endl;
       if( eleInEndcap(index_selected_ele) ) myfile2 << " (in ENDCAP)"<< endl;
       if( eleInGap(index_selected_ele)    ) myfile2 << " (in GAP)"   << endl;
       myfile2 << "             phi:  " << els_phi->at(index_selected_ele) << endl;
       myfile2 << "      ID     H/E:  " << els_hadOverEm->at(index_selected_ele) << endl;
       myfile2 << "      ID  detain:  " << els_dEtaIn->at(index_selected_ele) << endl;
       myfile2 << "      ID  dphiin:  " << els_dPhiIn->at(index_selected_ele) << endl;
       myfile2 << "      ID   sieie:  " << els_sigmaIEtaIEta->at(index_selected_ele) << endl;
       myfile2 << "           E/Pin:  " << els_eOverPIn->at(index_selected_ele) << endl;
       myfile2 << " chosen  |d0| to cut on:  " << get_d0("electron",index_selected_ele) << endl;
       ...
       myfile2 << "    tkIso (dr0.3):  " << els_dr03TkSumPt->at(index_selected_ele) << endl;
       myfile2 << "  ecalIso (dr0.3):  " << els_dr03EcalRecHitSumEt->at(index_selected_ele) << endl;
       myfile2 << "  hcalIso (dr0.3):  " << els_dr03HcalTowerSumEt->at(index_selected_ele) << endl;
       myfile2 << "   RelIso (dr0.3):  " << getRelIso(index_selected_ele) << endl;
       if(Npv>0)myfile2<<"|PVz-Ez| (EleZ): " <<fabs(pv_z->at(0) - els_vz->at(index_selected_ele) )<<", ( "<<  els_vz->at(index_selected_ele)<<" )"<<endl;
       if(m_useMisslayers)  myfile2 << "num missing layers:  " << els_innerLayerMissingHits->at(index_selected_ele) << endl;
       if( ConversionFinder(index_selected_ele) ) myfile2 << " -> conversion";
       else                      myfile2 << " -> not conversion";
       myfile2 << ", dist: "<<x_minDist << "dcot: " <<x_minDcot<<endl;
       myfile2 << "************************************" << endl;
       
       TVector2 e2v( els_px->at(index_selected_ele), els_py->at(index_selected_ele) );
       ...
       myfile2 << "   MT(e,calomet): " << a_mtw    << " GeV  (calomet: " << mets_et->at(0)   << " GeV), Phi: " 
	       << mets_phi->at(0) << endl;
       myfile2 << "   MT(e,pfmet):   " << a_mtw_pf << " GeV  (pfmet:   " << PFMets_et->at(0) << " GeV), Phi: "
	       << PFMets_phi->at(0) << endl;
       myfile2 << "   MT(e,tcmet):   " << a_mtw_tc << " GeV  (tcmet:   " << tcmets_et->at(0) << " GeV), Phi: "	       
	       << tcmets_phi->at(0) << endl;
       myfile2 << "************************************"<< endl;	   
	 ...
	 myfile2 << ", phi = " << jets[i].Phi();
	 //BBBBBBBB Add DeltaR(e,jet) info
	 if( iso_electrons.size() >= 1 ) { myfile2 << "  DR(e,j) = " << jets[i].DeltaR(iso_electrons.at(0)); }
	 myfile2 << endl;
       }      
       myfile2 << "************************************" << endl;
	 ...
	 if(nGoodEle > 0){
	   ...
	   if(nGoodIsoEle > 0){
	     ...
	     if(nGoodIsoEle == 1){
	       ...
	       /*	       
	       myfile5<<event<<":"<<els_et->at(index_selected_ele)<<":"<<els_eta->at(index_selected_ele)<<":"
		      <<getRelIso(index_selected_ele)<<":"<<get_d0("electron",index_selected_ele)<<endl;

	       if(1){
		 myfile4<<"Run: "<<run<<", Event: "<<event<< "Lumi: " <<lumiBlock <<", ET: "<<els_et->at(index_selected_ele)
			<<", eta: "<<els_eta->at(index_selected_ele)<<", dist: "<<x_minDist<<", dcot: "<<x_minDcot<<", htlep: "<<ht_lep
			<< ", MET: "<<this_met<<", MTW: "<< this_t1_mtw  << ", njet: "<<nGoodJet<<endl;}
	       */
	   
	       //if(  !m_useMisslayers || (m_useMisslayers && els_innerLayerMissingHits->at(index_selected_ele) <1) ){
	       //if( pass_missHits && !isConversion){ //missing hits
	       
	       //if(IsData()) myfile4<<run<<":"<<event<<":"<<x_minDist<<":"<<x_minDcot<<endl;
		 ...
		 if(!isZ){ // Z Veto
		   ...
		   //if(ntj>0) summaryOutput<< run<<":"<<event<<":"<<lumiBlock<< endl;
		   ...
		   if( pass_met ){  // MET
			 ...
			 myfile4.precision(4);
			 //myfile4<<run<<":"<<event<<endl;

			 
			 if(IsData() && iso_electrons.at(0).Et() > 120 && 0) {
			 ...
			 //myfile5.precision(4);
			 //myfile5<<"Run: "<<run<<", Event: "<<event<< "Lumi: " <<lumiBlock <<", ET: "<<els_et->at(index_selected_ele)  
			 	 //Et, iso, id, d0,
     ...
     if(m_doPostCutPlots && !isMuon && !isZ && fired_single_em && goodrun && Nels>0 && !isConversion){
	 ...
	 //note this was missing for some of the recent files, resulting in a high Et Electron 
	 if(IsData()  &&  m_ntupleHasSwissCross  &&  m_cutOnSwissCross
	    &&  eleInBarrel(i)  &&  els_isEcalDriven->at(i)>0 ) {
	   if( e_swissCross->at(i) > 0.95 ) continue;
	 }
	 ...
	 if( bp_et && bp_eta && bp_iso && bp_mh){ if( fabs(get_d0("electron",i))<fabs(bf_d0) ){bf_d0 = get_d0("electron",i);} }
       ...
       if(bf_et > 0) {  fillHisto_Njet_DataAndMC(  AN_et , bf_et, this_weight ); }
       if( bf_eta < 10) { fillHisto_Njet_DataAndMC( AN_eta , bf_eta,  this_weight );}
       if(bf_iso < 100 ){ fillHisto_Njet_DataAndMC( AN_iso , bf_iso,  this_weight );
         fillHisto_Njet_DataAndMC( AN_trkiso , bf_trkiso,  this_weight );
         fillHisto_Njet_DataAndMC( AN_ecaliso , bf_ecalIso,  this_weight );
         fillHisto_Njet_DataAndMC( AN_hcaliso,  bf_hcalIso,  this_weight );
       }
       if(bf_d0 <10 ){ fillHisto_Njet_DataAndMC( AN_d0 , fabs(bf_d0),  this_weight );}
       if( bf_mh < 10 ) { fillHisto_Njet_DataAndMC( AN_mh , bf_mh,  this_weight );}
       
        
     
       if(nGoodIsoEle == 1 && pass_missHits && !isConversion){//added conversion, 15-08-10
	
	 if(eleInBarrel(index_selected_ele) ){
	   fillHisto_Njet_DataAndMC( AN_HoE , els_hadOverEm->at(index_selected_ele) );
	   fillHisto_Njet_DataAndMC( AN_SigmaIetaIeta , els_sigmaIEtaIEta->at(index_selected_ele)  );
	   fillHisto_Njet_DataAndMC( AN_DelEta , els_dEtaIn->at(index_selected_ele) );
	   fillHisto_Njet_DataAndMC(  AN_DelPhi, els_dPhiIn->at(index_selected_ele) );
	 } else{
	   fillHisto_Njet_DataAndMC( AN_EE_HoE , els_hadOverEm->at(index_selected_ele) );
	   fillHisto_Njet_DataAndMC( AN_EE_SigmaIetaIeta , els_sigmaIEtaIEta->at(index_selected_ele)  );
	   fillHisto_Njet_DataAndMC( AN_EE_DelEta , els_dEtaIn->at(index_selected_ele) );
	   fillHisto_Njet_DataAndMC(  AN_EE_DelPhi, els_dPhiIn->at(index_selected_ele) );
	 }
	 ...
	 int ij = 0;
	 for(unsigned int i = 0; i<NPFJets2; ++i) {
	   if ( fabs(PFJets2_eta->at(i) ) < 2.4 ) {
	     TLorentzVector thisjet(PFJets2_px->at(i),PFJets2_py->at(i),PFJets2_pz->at(i),PFJets2_energy->at(i));
	     if( !jetNotNearElectron(thisjet, electrons) )continue;
	     if(ij==0) fillHistoDataAndMC( AN_1leadPFJet_pt , PFJets2_pt->at(i) );
	     if(ij==1) fillHistoDataAndMC( AN_2leadPFJet_pt , PFJets2_pt->at(i) );
	     if(ij==2) fillHistoDataAndMC( AN_3leadPFJet_pt , PFJets2_pt->at(i) );
	     if(ij==3) fillHistoDataAndMC( AN_4leadPFJet_pt , PFJets2_pt->at(i) );
	     if(ij==4) break;
	     ij++;
	   }
	 }
	 ...
	 for(unsigned int i = 0; i<Njets; ++i) {
	   if ( fabs(jets_eta->at(i) ) < 2.4 && 
		jets_id_hitsInN90->at(i) > 1 &&
		jets_id_fHPD->at(i) < 0.98 &&
		jets_emf->at(i) > 0.01) {
	     TLorentzVector thisjet(jets_px->at(i),jets_py->at(i),jets_pz->at(i),jets_energy->at(i));
	     if( !jetNotNearElectron(thisjet, electrons) ) continue;
	     if(ij==0) fillHistoDataAndMC( AN_1leadJet_pt , jets_pt->at(i) );
	     if(ij==1) fillHistoDataAndMC( AN_2leadJet_pt , jets_pt->at(i) );
	     if(ij==2) fillHistoDataAndMC( AN_3leadJet_pt , jets_pt->at(i) );
	     if(ij==3) fillHistoDataAndMC( AN_4leadJet_pt , jets_pt->at(i) );
	     if(ij==4) break;
	     ij++;
	   }
	 }
...
AN_4leadJet_pt
	 */
	 if(nGoodJet>2){
	   pair<double,double> res = compute_M3(jets);
	   fillHisto_Njet_DataAndMC( AN_M3, res.first );
	   fillHisto_Njet_DataAndMC( AN_M3MaxPT, res.second );
	 }

	 fillHisto_Njet_DataAndMC( AN_met, this_met );
	 fillHisto_Njet_DataAndMC( AN_HT, ht );
	 fillHisto_Njet_DataAndMC( AN_HTlep, ht_lep );
	 fillHisto_Njet_DataAndMC(AN_mtw, this_t1_mtw);
	 fillHisto_Njet_DataAndMC( AN_ElzPVz, fabs(pv_z->at(0) - els_vz->at(index_selected_ele) ) );
       }//end of iso ele requriement
...
//      if ( goodrun  &&  fired_single_em  &&  nGoodEle>0  && nGoodIsoEle<2 && pass_missHits &&  pass_met  &&
...
//      }
	 ...
	 m_RelIso = electrons_isoval.at(0);
...
//        }
     ...
     if(m_doPostCutPlots  && fired_single_em && goodrun &&  nGoodEle>0 &&  nGoodIsoEle<2 && !isMuon  &&  !isZ) {
       //should pass all selection except for conversion veto, MH, njets
       FillConversionTree(ii_GoodEle_mostIso);
      }
	 ...
	 if( m_RelIso > -100 && m_RelIso < 0 ) {
	   static int ppp = 0;
	   if(ppp<100) cout << "Signal Region: RelIso ("<< m_RelIso << ") is negative, reset to 0." << endl;
	   if(ppp==0) cout << "will print the first 100 such instances." << endl;
	   ppp++;
	   m_RelIso = 0;	  
	 }
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso,      m_RelIso,    this_weight );
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIsoUW,    m_RelIso,    1.0         );
	 

	 TVector2 an_e2v( els_px->at(ii_GoodEle_mostIso),els_py->at(ii_GoodEle_mostIso));
	 float an_this_t1_mtw = compute_mtw( an_e2v, met2v_t1 );
	 if(an_this_t1_mtw < 30){fillHisto_Njet_DataAndMC( AN_aes_reliso, getRelIso(ii_GoodEle_mostIso) );}


	 /// AAAAAAAAA
	 if(IsData()) printRelIsoList();

	 if( pass_4jets && m_RelIso < 0.1 ) nDebugIso++;


     
	 //---------------------------------------
	 // Fill m3 plots for W+jets estimation
	 //---------------------------------------
	 // Compute m3 for events with 3 jets or more, but 
	 // only plot m3 for event with at least 4 jets.
	 // Require at least 3 jest to reconstruct t->jjj
	 ...
	 for (unsigned int i=0; i<Nels; ++i){ //1st e loop
	   ...
	   for (unsigned int j=0; j<Nels; ++j){ //2nd e loop
	     ...
	     //if( els_robustLooseId->at(j) < 1 ) continue; //change to WP95, 18-08-10
	     if(!passEleID_VBTF_W95(j)) continue;
	 ...
	 ht_AES += jets.at(i).Pt();
       ...
       fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_before, m_RelIso, this_weight );
	 ...
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES, m_RelIso, this_weight ); //All AES cut
	 ...
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_justHT, m_RelIso, this_weight );

	 if( flag_AES_pass_tighterZveto_mee && flag_AES_pass_tighterZveto_mep ){
	   //cout << "amtb AES - met" << endl;
	   fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_minusMET, m_RelIso, this_weight );
	 }
       }
       // AES: N-1 (HT)
       if( flag_AES_pass_metcut ) {

	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_justMET, m_RelIso, this_weight );

	 if (flag_AES_pass_tighterZveto_mee && flag_AES_pass_tighterZveto_mep ){
	   //cout << "amtb AES - ht" << endl;
	   fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_minusHT, m_RelIso, this_weight );
	 }
	 
	 // AES: N-1 (tighter Z, both mee, mep)
	 if( flag_AES_pass_HTcut ){
	   //cout << "amtb AES - Z" << endl;
	   fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_minusTighterZ, m_RelIso, this_weight );
	 }
       }//pass AES_MET
       else { //event has high MET
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_justHighMET, m_RelIso, this_weight );
       }
       // AES: just tighter Z
       if( flag_AES_pass_tighterZveto_mee && flag_AES_pass_tighterZveto_mep ) 
	 fillHisto_Njet_DataAndMC( h_QCDest_CombRelIso_AES_justZ, m_RelIso, this_weight );
...
void ana::StudyRelisoNESatEachStage(){
  ...
  if( goodrun  &&  fired_single_em ) {
	...
	if(mm<10) {
	  cout << " info: negative CombRelIso: " << m_RelIso << endl;
	  ++mm;
	}
      }
      ...
      iso_fillHisto_NES( 8, m_RelIso, this_met, mige_inBarrel );
	...
	iso_fillHisto_NES( 9, m_RelIso, this_met, mige_inBarrel );

	if( !mige_isConversion ) { //this is a private variable

	  if(m_debug) cout << "-> Filling Reliso NES histograms, L4" << endl;

	  iso_fillHisto_NES( 10, m_RelIso, this_met, mige_inBarrel );
	}
      }
    }//nGoodEle > 0
  }//goodrun, trigger
...
void ana::fillHisto_Njet_DataAndMC( v2D_TH1& h, const float& value ) const {

  fillHisto_Njet_DataAndMC( h, value, this_weight );
...
//calls default conversionFinder behaviour, to determine dist/dcot values
//bool ana::ConversionFinder(  int index_selected_ele) {
//  return ConversionFinder(index_selected_ele, 0);
//}

bool ana::ConversionFinder2(  int index_selected_ele) {
  return ConversionFinder2(index_selected_ele, 0.02,0.02);
}

bool ana::ConversionFinder2(  int index_selected_ele, float this_dist, float this_dcot) {
  ...
  tr_conversionPartnerTrack_ML = -1;
  pair <float,float>ConvCentre;

  for (unsigned int j=0; j<Ntracks; ++j){//loop over the track collection 
    
    if( (abs(mytrackref)-j)==0 && els_shFracInnerHits->at(index_selected_ele) > 0.45) continue;//works if trackref is valid or not (invalid it will be less than zero, which j is not)
    ...
    ConvCentre.first = (tk2r+dist/2.0)*(tk1x-tk2x)/distmag + tk2x;
    ConvCentre.second = (tk2r+dist/2.0)*(tk1y-tk2y)/distmag + tk2y;

    if( fabs(dist) < this_dist && fabs(dcot) < this_dcot)
	...
	if(mlPass){
	  ...
	  //if(m_doPostCutPlots){
	  tr_conversionPartnerTrack_ML = (NumMissLayers - tracks_innerLayerMissingHits->at(j) );
	  tr_conversionPartnerTrack_MHits = tracks_innerLayerMissingHits->at(j);
          tr_Conv_2Dradius = sqrt ((ConvCentre.first * ConvCentre.first) + (ConvCentre.second * ConvCentre.second) );
          tr_Conv_centreX = ConvCentre.first;
          tr_Conv_centreY = ConvCentre.second;
          tr_Conv_theta = theta1;
	  //}
	  tr_Conv_Mdist = dist;
	  tr_Conv_Mdcot = dcot;
	  //if(event==19921112){cout<<"mine: dist: "<<dist<<", dcot: "<<dcot<<endl;}
	  break;
	}
      }
...
bool ana::ConversionFinder(  int index_selected_ele) {
  return ConversionFinder(index_selected_ele, 0.02,0.02);
}

bool ana::ConversionFinder(  int index_selected_ele, float this_dist, float this_dcot) {
	
  //changes made to this compared to ConversionFinder2:
  // - only the track which gives the minimalist dcot is considered with dist
  // - d0dum is used for the IP (gsftrack.d0, track.d0, as in Puneeth's code)
  // - extra condition applied at start of track loop, which actually was a bug, so should be adopted above
  // - delta R set to 0.5. I think it should remain at 0.3 to conserve signal - little affect on conversions. 	
	...
	float minDist=-9999.,minDcot=-9999.;
	...
	if(mytrackref>=0 && els_shFracInnerHits->at(index_selected_ele) > 0.45){//use CTF params
		...
		eta1 = tracks_eta->at(mytrackref);
		charge1 = tracks_chg->at(mytrackref);
		d0 = tracks_d0dum->at(mytrackref);//compute_d0("track",mytrackref); //****** TL 12MAY ***** check with Frankie    
		tk1Curvature = -0.3*bfield*charge1/( 100*( tracks_pt->at(mytrackref) ) );
		theta1 = tracks_theta->at(mytrackref);
		if(m_useMisslayers) NumMissLayers = tracks_innerLayerMissingHits->at(mytrackref); 
	}
		...
		eta1 = els_tk_eta->at(index_selected_ele);
		charge1 = els_tk_charge->at(index_selected_ele);
		d0 = els_d0dum->at(index_selected_ele);//compute_d0("electronConv",index_selected_ele); //****** TL 12MAY ***** check
		tk1Curvature = -0.3*bfield*charge1/( 100*( els_tk_pt->at(index_selected_ele) ) );
		theta1 = els_tk_theta->at(index_selected_ele);
		if(m_useMisslayers) NumMissLayers = els_innerLayerMissingHits->at(index_selected_ele);
    }
	...
	for (unsigned int j=0; j<Ntracks; ++j){//loop over the track collection 
	  ...
	  if( (abs(mytrackref)-j)==0  && els_shFracInnerHits->at(index_selected_ele) > 0.45) continue;
	  //works if trackref is valid or not (invalid it will be less than zero, which j is not)
		phi2 = tracks_phi->at(j);
		eta2 = tracks_eta->at(j);
		...
		EleTrackDelR = calcDeltaR(phi1, eta1, phi2, eta2);
		if(EleTrackDelR > 0.5) continue;
		
		tk2Curvature = -0.3*bfield*tracks_chg->at(j)/( 100*( tracks_pt->at(j) ) );
		tk2r = fabs(1/tk2Curvature);
		
		d02 = tracks_d0dum->at(j);//compute_d0("track",j); //****** TL 12MAY ***** check with Frankie
		...
		//first find minimum dcot track, then use this for conversion candidate. 
		if(fabs(dcot)<fabs(minDcot)){minDcot = dcot;minDist = dist;}
	...
	x_minDist = minDist;
	x_minDcot = minDcot;
	//if(event==19921112){cout<<"Pun: dist: "<<minDist<<", dcot: "<<minDcot<<endl;}
	if(fabs(minDcot)<this_dcot && fabs(minDist)<this_dist){isthisConversion=true;}
...
void ana::reco_hadronicTop_highestTopPT(const vector<TLorentzVector>& jetColl, const int& nGoodIsoEle ){
  ...
  if ( nGoodIsoEle == 1 ) { // this is signal region
    ...
    if (jetColl.size()>=4) {
      ...
      if( IsData() ) { myfile3 << "m3 4mj Signal Region"<< endl; printRelIsoList(); myfile3 << "m3" << endl; }
    }
    if( jetColl.size()==3){
      fillHistoDataAndMC( h_m3_3j,      m3 );
      h_hadTop_maxPT_pt_3j->Fill(max_top_PT, this_weight);
      h_hadTop_maxPT_mass_3j->Fill(m3, this_weight);
    }
    ...
    if ( jetColl.size() >= 4 ) {
      ...
      if( IsData() ) { myfile3 << "m3 4mj Control Region"<< endl; printRelIsoList(); myfile3 << "m3" << endl; }
    }//>=4j

    if( jetColl.size()==3){
      fillHistoDataAndMC( h_m3_control_3j,      m3 );
      h_hadTop_maxPT_pt_nonIso_3j->Fill( max_top_PT, this_weight );
      h_hadTop_maxPT_mass_nonIso_3j->Fill( m3, this_weight );
    }
...
bool ana::EstimateWjets(const string inputFile_data, const string inputFile_mc) {
  ...
  //TH1D *h_m3_tt   = (TH1D*)fmc->Get(Form("%s/m3_3j__ttbar",dirW.c_str())); 
  //TH1D *h_m3_wj   = (TH1D*)fmc->Get(Form("%s/m3_3j__wj",   dirW.c_str())); 
  ...
  if (IsData()==false) { //MC
    ...
    //h_m3_zj     = (TH1D*)fmc->Get(Form("%s/m3_3j__zj",dirW.c_str()));              
    //h_m3_stop   = (TH1D*)fmc->Get(Form("%s/m3_3j__singleTop",dirW.c_str()));       
    //h_m3_qcd_control = (TH1D*)fmc->Get(Form("%s/m3_control_3j__QCD",dirW.c_str()));
  }



  TH1D *h_m3_MCdata_control = (TH1D*)fdata->Get(Form("%s/hadTop_maxPT_mass_nonIso_4j",dirW.c_str()));
  //TH1D *h_m3_MCdata_control = (TH1D*)fdata->Get(Form("%s/hadTop_maxPT_mass_nonIso_3j",dirW.c_str()));
  TH1D *control_all = (TH1D*)h_m3_MCdata_control->Clone("control_all");
  //TH1D *control_all = (TH1D*)h_m3_qcd_control->Clone("control_all"); 
  ...
  */
  ...
  //TH1D *h_m3_wj2 = (TH1D*)fmc2->Get("Wjets_estimation/m3_wj")->Clone("model_wj");
    ...
    //temp_tt = (TH1D*)fmc2->Get(Form("%s/m3__ttbar",dirW.c_str()));
    //temp_wj = (TH1D*)fmc2->Get(Form("%s/m3__wj",dirW.c_str()));
    //temp_tt = (TH1D*)fmc2->Get(Form("%s/m3_3j__ttbar",dirW.c_str()));
    //temp_wj = (TH1D*)fmc2->Get(Form("%s/m3_3j__wj",dirW.c_str()));    
  ...
  */
  ...
  //use qcd estimate unless it has been set to zero, in which case use true value
  nqcd_est = m_QCD_estimateForM3?m_QCD_estimateForM3:nqcd_true;
  ...
  */
  ...
  for ( int n=0; n<nfit; ++n ) {
    ...
    if(IsData()){
      ...
      cout<<"Total events in signal region: "<<n_event_obs<<endl;
      cout<<"ntt fitted: "<<ntt_fit<<" +/- "<<ntt_fiterr<<endl;
      cout<<"nwj fitted: "<<nwj_fit<<" +/- "<<nwj_fiterr<<endl;
      cout<<"nqcd fitted: "<<nqcd_fit<<" +/- "<<nqcd_fiterr<<endl;
      cout<<"nstop fitted: "<<nstop_fit<<" +/- "<<nstop_fiterr<<endl;
...
void ana::PrintEventCounter() const {
  ...
  cout<<"End of event counter"<<endl;
  ...
  if(flag[1]){
    cout<<"TTBAR"<<endl;
    cout << setw(16)<<""<<  setw(10) << "0j"   	 << setw(23) << "1j"   	 << setw(23) << "2j"   	 << setw(23) << "3j"   	 << setw(23) << ">=4j" 
	 << setw(23) << "allj" << "\n" << endl;
    for(int i=0; i < nstage; ++i){
      cout <<ve.at(i)<<setw(10);
      int    sumjet=0;
      double sumjet2=0;
      for(int j=0; j < ntjet; ++j){
	int sumtype=0;
	...
	for(int k=kbegin; k <=kend ; ++k){
	  sumtype+=e_plus_jet[i][j][k];
	  sumtype2+=e_plus_jet_weighted[i][j][k];
	}
	printf("%10d (%10.2f)", sumtype, sumtype2);
	sumjet += sumtype;//e_plus_jet[i][j][k];
	sumjet2 += sumtype2;//e_plus_jet_weighted[i][j][k];
      }
      printf("%10d (%10.2f)\n", sumjet, sumjet2);
    }
  }
  if(flag[11]){
    ...
    for(int i=0; i < nstage; ++i){
      cout <<ve.at(i)<<setw(10);
      int    sumjet=0;      double sumjet2=0;
      for(int j=0; j < ntjet; ++j){
	int sumtype=0;	double sumtype2=0.;
	for(int k=kbegin; k <=kend ; ++k){
	  sumtype+=e_plus_jet[i][j][k];	  sumtype2+=e_plus_jet_weighted[i][j][k];
	}
	printf("%10d (%10.2f)", sumtype, sumtype2);
	sumjet += sumtype;//e_plus_jet[i][j][k];
	sumjet2 += sumtype2;//e_plus_jet_weighted[i][j][k];
...
void ana::DrawSignalAcceptanceTable( vector<string> ve ) const {
  ...
  cout << endl << "%------------------------------------------------------------------";
  cout << endl << "%------------------------------------------------------------------";
  cout << endl << "      Acceptance for ttbar MC (unweighted)";
  cout << endl << "%------------------------------------------------------------------";
  cout << endl << "%------------------------------------------------------------------";
  ...
  for(int i=0; i<ncutshown; ++i) { //nstage
    ...
    if(ve.at(i)=="!CONV") {startnj=3;
      ve.at(i) = njets_text;i--;
    }
    else if(startnj==3) {startnj=4; njets_text = "$\\ge$4Jets";
      ve.at(i) = njets_text;
      i--;
    }
...
void ana::PrintErrorTable_MC(){
  ...
  //-------------------------------
  //for(int i=0; i<7; ++i){
  for(int i=0; i<11; ++i){//changed to after conv veto
    ...
    */
    ...
    totalerr = 0;
    for(int j=0; j<5; ++j){
      total    += Pjet_Sum_pass_weighted2[i][j];
      totalerr += TMath::Power( Pjet_Sum_Effic_unc_pos[i][j], 2 );
    }
    printLine(myfile, total, sqrt(totalerr));
    TotalEvents     += total;
    TotalErrorPos   += totalerr;
  ...
  //
  //------------------------
  for(int i=10; i<ncutshown; ++i){ //cut
...
string ana::printEleID() const {
  ...
  default:          return "VBTF_W70 (Default)";
  }
}
//---------------------------------------------------------------------------------------------
...
bool ana::passEleID_VBTF_W95(const uint& i) const{
  if( eleInBarrel(i) ){
    if( els_sigmaIEtaIEta->at(i) < 0.01  &&
        fabs(els_dPhiIn->at(i))  < 0.8  &&
	fabs(els_dEtaIn->at(i))  < 0.007 &&
        els_hadOverEm->at(i)     < 0.15 ) return true;
    else return false;
  }
  else if(eleInEndcap(i)){
    if( els_sigmaIEtaIEta->at(i) < 0.03  &&
        fabs(els_dPhiIn->at(i))  < 0.7  &&
        fabs(els_dEtaIn->at(i))  < 0.01 &&
        els_hadOverEm->at(i)     < 0.07 ) return true;
    else return false;
  }
  return false;
}
...
bool ana::passHLT() const {
  if ( m_runOnV4ntuples ){
    ...
    if ( IsData()==false ){//use emulated photon15 for MC and older data
      //return pass_photon15;
      //Need some (emulated?) trigger for MC, but what?
      //return pass_photon10_TO20;
      return true;//apply no trigger for MC
    }    

    else { //REAL DATA
      if( run <= 140041 ){//use emulated photon15 for older data
	//return pass_photon15;
	return (bool)HLT_Ele10_LW_L1R;
      }    
      else if (  run > 140041 && run <= 143962){ //use HLT_Photon15 for newer data
	return (bool)HLT_Ele15_SW_L1R;
      }
      else if(run > 143962 && run <= 146427){
	return (bool)HLT_Ele15_SW_CaloEleId_L1R;
      }
      else if(run > 146427 && run <= 147116 ){
	return (bool)HLT_Ele17_SW_CaloEleId_L1R;
      }              
      else if(run > 147116 && run <= 148818){
	return (bool)HLT_Ele17_SW_TightEleId_L1R;
      }              
      else if(run > 148818){
	return (bool)HLT_Ele22_SW_TighterEleId_L1R_v2;
      }
      else{cout<<"WARNING, invalid run number for data HLT"<<endl;}
      //else {return true;}//for later runs, return true for all due to prescale. 
    }
...
void ana::DoBTagging(const vector<TLorentzVector>& electrons){
  ...
  if (m_jetAlgo=="pfjet") return;//currently code doesn't work for non-calo jets
  // .. TO DO ...
  ...
  for(unsigned int i=0; i< Njets; ++i){
   ...
   if ( jets_pt->at(i) > JET_PTCUT   &&
	...
	if(!IsData()){if(jets_btag_secVertex->at(i)  > discut_SSV)  m_nbtag_SSV++;}
	else{if(jets_btag_ssvHE->at(i)  > discut_SSV)  m_nbtag_SSV++;}
      }//DR
    }//quality cuts
  }
...
void ana::FillConversionTree(int selected_ele_index){
  tr_eleEt = els_et->at(selected_ele_index);
  tr_eleD0= fabs(get_d0("electron",selected_ele_index));//compute_d0("electron",selected_ele_index);
  tr_eleTrkIso= els_tIso->at(selected_ele_index);
  tr_eleEcalIso= els_dr03EcalRecHitSumEt->at(selected_ele_index);
  tr_eleHcalIso= els_dr03HcalTowerSumEt->at(selected_ele_index);
  tr_eleRelIso= getRelIso(selected_ele_index);
  tr_eleEta= els_eta->at(selected_ele_index);
  tr_elePhi= els_phi->at(selected_ele_index);

  tr_eleML= els_innerLayerMissingHits->at(selected_ele_index);
  tr_Conv_Mdist = 9999;//these will be set in second call to conversionFinder2 below
  tr_Conv_Mdcot = 9999;
  tr_conversionPartnerTrack_ML = -9999.0;
  tr_conversionPartnerTrack_MHits = -9999.0;
  tr_Conv_2Dradius = -9999.0;
  tr_Conv_centreX = -9999.;
  tr_Conv_centreY = -9999.;
  tr_Conv_theta = -9999.;


  tr_eleConverted_Pun = static_cast<float>(ConversionFinder( selected_ele_index));
  tr_eleConverted_Mine = static_cast<float>(ConversionFinder2( selected_ele_index));
  tr_eleConverted_Pun2 = static_cast<float>(ConversionFinder( selected_ele_index,0.04,0.03 ));
  //tr_convRadius will be from the following:
  tr_eleConverted_Mine2 = static_cast<float>(ConversionFinder2( selected_ele_index,0.04,0.03 ));
  /*
  tr_eleConverted_Pun = static_cast<bool>(ConversionFinder( selected_ele_index));
  tr_eleConverted_Mine =static_cast<bool>(ConversionFinder2( selected_ele_index));
  tr_eleConverted_Pun2 =static_cast<bool>(ConversionFinder( selected_ele_index,0.04,0.03 ));
  tr_eleConverted_Mine2 =static_cast<bool>(ConversionFinder2( selected_ele_index,0.04,0.03 ));
  */
  //get the truth value
  if(!IsData())tr_eleGenMatch = MCTruthMatch( els_eta->at(selected_ele_index), els_phi->at(selected_ele_index));
  tr_shFracInnerHits = els_shFracInnerHits->at(selected_ele_index);
  
  tr_mctype = mctype;
  tr_weight = this_weight;
  tr_met = mets_et->at(0);
  tr_njet = nGoodJet;
  int mytrackref  = static_cast<int>( els_closestCtfTrackRef->at(selected_ele_index) );
  if(mytrackref>=0 && els_shFracInnerHits->at(selected_ele_index) > 0.45){
    tr_eleCTFML=tracks_innerLayerMissingHits->at(mytrackref);}
  //tr_MuonVeto = isMuon;
  //tr_Zveto = isZ;
  tr_passSelection = 0;
  //if(goodrun && !isMuon && !isZ) tr_passSelection = 1;//trig,goodrun,1isoele,!mu!z, but could have mh,conv
  tr_event = event;
  mytree->Fill();
}
